...
class Watch(models.Model):
    ...
    name = models.CharField(
        max_length=100, unique=True, verbose_name="Nombre del Reloj (Tipografía Audaz)"
    )
    # Descripción para el texto discreto
    short_description = models.TextField(
        max_length=200, verbose_name="Descripción Breve (Texto discreto)"
    )
    ...
    model_3d_url = models.URLField(
        max_length=300, blank=True, null=True, verbose_name="URL del Modelo 3D (.glb)"
    )
    # Color de acento (ej. #FF5733) para el botón o el título
    highlight_color = models.CharField(
        max_length=7, default="#FF00D4", verbose_name="Color de Acento (Neón)"
    )
    ...
    is_experimental_hero = models.BooleanField(
        default=False, verbose_name="¿Es apto para el Hero experimental?"
    )
